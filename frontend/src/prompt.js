import { getWord, getWordMeaningAndCommentary,
  getWordsAndMeaningsForShlokaLine } from './data';

function meaningInBraces(meaning) {
...
export function wordsBefore({ shlokaNum, lineNum, wordIndex }) {
  let wordsWindow;
  if (wordIndex > 2) {
    wordsWindow = getWordsAndMeaningsForShlokaLine(shlokaNum, lineNum)
      .slice(wordIndex - 3, wordIndex);
  } else {
    const shlokaWithPrevLine = lineNum > 1? shlokaNum : shlokaNum - 1;
    const prevLine = lineNum > 1 ? 1 : 2;
    if (shlokaWithPrevLine > 0) {
      wordsWindow = getWordsAndMeaningsForShlokaLine(shlokaWithPrevLine, prevLine)
        .slice(- (3 - wordIndex))
        .concat(getWordsAndMeaningsForShlokaLine(shlokaNum, lineNum)
        .slice(0, wordIndex));
    } else {
      wordsWindow = getWordsAndMeaningsForShlokaLine(shlokaNum, lineNum).slice(0, wordIndex);
    }
  }
  if (wordsWindow?.length > 0) {
    const csWords = wordsWindow.map(wordAndMeaning => 
      `- ${wordAndMeaning.word.trim()} (${wordAndMeaning.meaning.trim()})`).join('\n');
    return `The names before this are:\n${csWords}\n`;
  }
  return '';
}
...
export function wordsAfter({ shlokaNum, lineNum, wordIndex }) {
  let nextWord;
  const wordsInLine = getWordsAndMeaningsForShlokaLine(shlokaNum, lineNum);
  if (wordIndex < wordsInLine.length - 1) {
    ...
    nextWord = getWordsAndMeaningsForShlokaLine(shlokaWithNextLine, nextLine)[0];
  }
  return nextWord ? `The name after this is ${nextWord.word.trim()} (${nextWord.meaning.trim()})\n` : '';
}
...
export function subjectOfPrompt(word, meaning, commentary) {
  return `The word "${word}"${meaningInBraces(meaning)} is explained as "${commentary}"`;
}
